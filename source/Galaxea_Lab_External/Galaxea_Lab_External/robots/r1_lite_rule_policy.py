...
articulation, and the grasp/mount trajectories were re-tuned for
R1_Lite's geometry (see docs/r1_lite_integration.md).

Everything that depends on the robot's frame conventions (EE link name,
gripper axis, "gripper down" quaternion, wrist-roll joint, fingertip
extension) is exposed as class attributes at the top of
``R1LiteRulePolicy`` so other Galaxea robots can subclass it — see
``r1_pro_rule_policy.py`` for the R1Pro variant.
"""
...
class R1LiteRulePolicy:
    # ------------------------------------------------------------------
    # Robot-frame parameters. Subclasses for other Galaxea robots override
    # these (see r1_pro_rule_policy.py); everything below reads them so the
    # task logic itself stays robot-agnostic.
    # ------------------------------------------------------------------
    #: Suffix of the arm body used as the differential-IK end-effector target.
    EE_LINK_SUFFIX: str = "_arm_link6"
    #: Suffix of the independently driven gripper finger joint (the other finger mimics it).
    GRIPPER_JOINT_SUFFIX: str = "_gripper_finger_joint1"
    #: Unit vector, in the EE link's local frame, along which the gripper/fingers extend.
    GRIPPER_AXIS_LOCAL: tuple[float, float, float] = (1.0, 0.0, 0.0)
    #: Quaternion (w, x, y, z) that rotates the EE link so ``GRIPPER_AXIS_LOCAL`` points world -Z
    #: (gripper pointing straight down). +90 deg about Y for R1_Lite's link6 +X gripper.
    ...
    #: Index (within the arm joint list) of the joint that spins the gripper about its own axis.
    #: Used by the joint-space "wiggle to mesh teeth" step; ignored when ``ROTATE_VIA_IK``.
    GRIPPER_ROLL_JOINT_INDEX: int = 5
    #: When True, the mesh-teeth wiggle rotates the IK target about the vertical TCP axis
    #: instead of driving a single wrist joint (needed for 7-DOF wrists whose last joint is
    #: not aligned with the gripper axis).
    ROTATE_VIA_IK: bool = False
    #: Distance from the finger-link origins to the grasp point along ``GRIPPER_AXIS_LOCAL``.
    ...
    obj_dict: dict):
        ...
        # Distance from finger_link origin to actual fingertip along the gripper axis.
        ...
        self.time_step_2 = self.PHASE_TIME_SCALE * torch.tensor([0.0, 1.5, 1.5, 0.5, 0.5], device=sim.device)
        ...
        self.time_step_9 = self.PHASE_TIME_SCALE * torch.tensor([0.0, 1.5, 1.5, 5.0, 0.5, 0.5], device=sim.device)
    ...
    def _compute_tcp_offset(self):
        ...
        IK targets the EE link (``EE_LINK_SUFFIX``), but the gear has to end up
        between the fingertips. We average the two finger origins, express the
        midpoint in the EE link's local frame, push it ``fingertip_extension``
        further along ``GRIPPER_AXIS_LOCAL``, and rotate that local offset into
        the world frame using ``GRIPPER_DOWN_QUAT`` (the EE orientation used for
        every grasp). The EE target is then ``TCP_target - offset_world``, i.e.

            TCP_offset_x = -offset_world.x
            TCP_offset_z = -offset_world.z

        For R1_Lite (gripper along link6 +X, +90 deg-Y down-quat) this reduces to
        the previous closed form ``TCP_offset_z = Lx + ext``, ``TCP_offset_x = -Lz``.
        """
        robot = self.scene["robot"]
    ...
    def _rotate_about_tcp_via_ik(self,
        ...
        """IK step that spins the gripper by ``angle_rad`` about the vertical axis
        through the TCP while keeping the TCP in place (used when ``ROTATE_VIA_IK``).

        ``ee_target_position`` already includes the world-frame TCP offset, so the
        EE orbits the TCP: the horizontal part of that offset is rotated by the
        same yaw that is applied to the orientation.
        """
    ...
    def move_robot_to_position(self,
        ...
        arm_key = self._arm_key(arm_entity_cfg)
    ...
    def pick_up_target_gear(self,
        ...
        # R1_Lite: +90 deg about Y (gripper along link6 +X). R1's gripper extends along
        # link6 +Z, so the R1 policy uses [0, 1, 0, 0] (180-X) instead.
        target_orientation, target_position = torch_utils.tf_combine(
    ...
    def mount_gear_to_target(self,
        ...
        if gear_id == 6:
            ...
            # roll/pitch matches the picked pose, then add the same "down" rotation.
            target_orientation, target_position = torch_utils.tf_combine(
        ...
        if self.count >= count_step[0] and self.count < count_step[1]:
            ...
            if self._phase_started(phase_start):
    ...
    def mount_gear_to_target_and_rotate(self,
        ...
        if self.count >= count_step[2] and self.count < count_step[3]:
            ...
            if self.ROTATE_VIA_IK:
                # 7-DOF wrists (R1Pro): no single joint is aligned with the gripper
                # axis, so spin the IK target about the vertical TCP axis instead.
